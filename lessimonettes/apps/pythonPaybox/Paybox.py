...
class Transaction:
	...
	def __init__(self, production=True, PBX_TOTAL=None, PBX_CMD=None, PBX_PORTEUR=None, PBX_TIME=None, PBX_REPONDRE_A=None):
		self.production = production
		if self.production:
			self.action = 'https://tpeweb.paybox.com/cgi/MYchoix_pagepaiement.cgi'
			self.SECRET = SECRETKEYPROD
		else:
			self.action = 'https://preprod-tpeweb.paybox.com/cgi/MYchoix_pagepaiement.cgi'
			self.SECRET = SECRETKEYTEST
		
		self.MANDATORY = {
			'PBX_SITE': PBX_SITE,			# SITE NUMBER (given by Paybox)
			'PBX_RANG': PBX_RANG,			# RANG NUMBER (given by Paybox)
			'PBX_IDENTIFIANT': PBX_IDENTIFIANT,	# IDENTIFIANT NUMBER (given by Paybox)
			'PBX_TOTAL': PBX_TOTAL,				# Total amount of the transaction, in cents
			'PBX_DEVISE': '978',				# Currency of the transaction
	...
	def verify_notification(self, response_url, order_total, verify_certificate=True):
		...
		query = urllib.parse.parse_qs(message) 	 	# dictionnary
	...
	def verify_certificate(self, message, signature):
		...
		# digest the message
		h = SHA.new(message_without_sign.encode('utf-8'))
